...
const resolutionMechanic = {
  name: `Resolution Mechanic`,
  description: `The game uses a card-based resolution mechanic instead of dice to provide a unique probability curve and allow for more strategic play. The card system introduces memory, tension, and the potential for players to track which cards have been played, rewarding skillful planning and risk assessment.`,
  rules: [
    {
      name: `Card Value Assignment`,
      description: `Each card in the custom deck has a specific value:

${deckComposition}`,
    },
    {
      name: `Card Flip Process`,
      description: `When a random outcome is required, the active player MUST flip two cards from their custom deck. The sum of their values, plus any relevant modifiers, determines the result.
...
**Important:** In contests (like initiative), all participants flip cards. In attacks, only the attacker flips cards unless a specific ability allows the defender to flip cards.`,
    ...
    {
      name: `Deck Cycling`,
      description: `Card management follows these rules:
...
- This discarded card is not revealed and CANNOT be examined`,
    ...
    {
      name: `Tie-Breaking Procedure`,
      description: `In case of tied totals between two or more sides, follow this procedure:

1. **Check Highest Suit:** The side with the highest-ranking suit among all flipped cards wins. Suit dominance is: Red ${SUIT_SYMBOLS.JOKER} > ${SUIT_SYMBOLS.SPADES} Spades > ${SUIT_SYMBOLS.HEARTS} Hearts > ${SUIT_SYMBOLS.DIAMONDS} Diamonds > ${SUIT_SYMBOLS.CLUBS} Clubs > Black ${SUIT_SYMBOLS.JOKER}.
...
4. **Exact Match:** If two or more sides flipped the exact same two cards, discard the flipped cards and repeat the card flip process to resolve the tie.`,
    ...
    {
      name: `Probability Awareness`,
      description: `Players SHOULD be aware that the card system produces sums from -3 to +7 with specific probabilities:
...
const turnStructure = {
  name: `Turn Structure`,
  description: `Rules governing the overall structure and flow of each turn.`,
  rules: [
    {
      name: `Turn Steps`,
      description: `Each turn follows these sequential steps:
...
Players MUST complete each phase before proceeding to the next.`,
    },
    {
      ...
      description: `A player MAY choose to pass their activation instead of selecting a character if the opponent has more characters left to activate.
`,
    },
    {
      name: `End Turn Conditions`,
      description: `The turn ends when one of the following conditions is met:
...
const activationRules = {
  ...
  description: `Rules governing character activation and action token usage.`,
  rules: [
    {
      name: `Character Activation Declaration`,
      description: `When it is a player's turn to activate, they MUST declare which specific character they are activating before taking any actions.
...
- Declaration is binding - the player CANNOT change to a different character once declared`,
    ...
    {
      name: `Action Token System`,
      description: `Characters use Action Tokens to perform abilities during their activation.
...
const gameComponents = {
  ...
  description: `Required materials and components needed to play Ascension: Legacy of Conflict.`,
  rules: [
    {
      name: `Character Requirements`,
      description: `All character miniatures MUST be between 25mm and 170mm base size. Character size directly impacts gameplay, including movement, line of sight, and area of effect calculations. Players MUST agree on character sizes before game begins`,
    },
    {
      name: `Terrain Requirements`,
      description: `Players MUST provide terrain pieces to create a dynamic battlefield. Terrain MAY include buildings, forests, obstacles, and other environmental features.
...
- Terrain affects line of sight, movement, and combat as defined in terrain interaction rules`,
    },
    {
      ...
      description: `Players SHOULD have appropriate measuring tools for accurate gameplay.
...
- Movement measurements are done in millimeters as this matches common base sizing standards`,
    ...
    {
      name: `Specialty Card Deck`,
      description: `Each player needs a custom 18-card deck with specific composition replacing traditional dice mechanics.

${deckComposition}

**Suit Distribution:**

Each Ace, Two, Three, and Jack MUST have one card of each suit: ${SUIT_SYMBOLS.SPADES} Spades, ${SUIT_SYMBOLS.HEARTS} Hearts, ${SUIT_SYMBOLS.DIAMONDS} Diamonds, and ${SUIT_SYMBOLS.CLUBS} Clubs.`,
    },
    {
      name: `Token Requirements`,
      description: `Players MUST provide various tokens to track game states and effects.
...
- Players SHOULD use consistent token types throughout the game`,
    ...
    {
      name: `Rule Priority and Conflicts`,
      description: `When rules conflict or create ambiguity, follow this resolution hierarchy.
...
const playerAndTeamRules = {
  name: `Player and Team Rules`,
  description: `Rules for player configurations, initiative, and team coordination that apply to all games.`,
  rules: [
    {
      name: `Supported Player Configurations`,
      description: `The game accommodates flexible player counts and team arrangements:
...
The game is designed to accommodate all players present, with systems to help balance experience and skill levels.`,
    ...
    {
      name: `Initiative Order and Team Coordination`,
      description: `Initiative and activation order rules for all games:
...
- Eliminated players MAY still participate in team discussions unless scenario rules specify otherwise`,
    ...
    {
      name: `Force Point Balancing`,
      description: `Balancing mechanism for asymmetric player counts and experience levels:
...
const movementRules = {
  name: `Movement Rules`,
  description: `Rules governing character movement and stride usage during Move Actions.`,
  rules: [
    {
      name: `Move Action Basics`,
      description: `Move Action is the default action available to all characters and does not need to be purchased during character creation.
...
- If there is already an Action Token on the Move Action's token slot, the character CANNOT take the action, just like all other actions`,
    },
    {
      ...
      description: `When a character performs movement, follow these steps for each Stride spent:
...
**Example:** A character with a 40mm x 25mm oval base moves straight forward using 40mm per Stride. Moving sideways or diagonally uses 25mm per Stride.`,
    ...
    {
      name: `Terrain Interaction`,
      description: `Characters can interact with terrain during movement using these rules:
...
**Example:** A Size 1 character climbing Size 3 terrain spends 2 Strides (3 - 1 = 2).`,
    ...
    {
      name: `Special Stride Actions`,
      description: `Characters can spend Strides for special actions during Move Actions:
...
const gameFlow = {
  name: `Game Flow`,
  description: `Core mechanics governing how the game is played from turn to turn.`,
  rules: [
    playerAndTeamRules,
    turnStructure,
    activationRules,
...
const attackActionResolution = {
  name: `Attack Action Resolution Sequence`,
  description: `When a character performs an Attack Action, follow this step-by-step process:
...
const terrainRules = {
  name: `Terrain Rules`,
  description: `Rules governing terrain interaction, destruction, and environmental effects.`,
  rules: [
    {
      name: `Cover and Line of Sight`,
      description: `Terrain affects visibility and protection based on relative sizes:
...
**Obscured:** No line of sight if terrain Size is greater than or equal to Character Size`,
    ...
    {
      name: `Terrain Destruction and Interaction`,
      description: `Rules for attacking and destroying terrain features:
...
- CANNOT be targeted or destroyed unless scenario rules specify otherwise`,
    ...
    {
      name: `Specialized Terrain Features`,
      description: `Rules for interactive and environmental terrain:
...
const specialRules = {
  name: `Special Rules`,
  description: `Additional rules covering temporary conditions and forced movement effects.`,
  rules: [
    {
      name: `Temporary Conditions`,
      description: `Status effects that can be inflicted by attacks and abilities. Each condition can stack up to 2 times. All temporary conditions stack their effect strength except Damage Over Time (DOT). All stacks must be removed before DOT's effect stops.
...
- Stacks: Multiple applications do NOT increase Wounds suffered, but all stacks must be removed before effect stops`,
    ...
    {
      name: `Forced Movement Effects`,
      description: `Rules for pushing characters and resulting damage:
...
const CoreRules = {
  name: `Core Rules`,
  description: `Core Rules for Ascension: Legacy of Conflict`,
  rules: [
    gameComponents,
    resolutionMechanic,
    gameFlow,
    attackActionResolution,
    movementRules,
    terrainRules,
    specialRules,
  ],
